...
import { useGetSignupMutation } from '../services/ProductsApi'
...
const SignUp = () => {
    ...
    const [Signup, { data, isSuccess, isLoading, isError }] = useGetSignupMutation()
    ...
    return (
        <div className='flex justify-center min-h-screen w-11/12 items-center m-auto '>
            <form
                ...
                <h2 className='text-gray-500 font-bold text-3xl flex border-l-8 px-2 border-l-slate-500 w-fit  justify-center items-center  m-auto'><BsPersonFill className='mt-1 ' size={30} fontWeight={900} />Signup</h2>
                <div className='mt-10'>
                    {err ? <div hidden={err} className={'text-red-600  flex items-center gap-1'}>
                        <BiErrorCircle size={20} />
                        Email or Password is invalid
                    </div> : null}
                    <h2 className='m-1 text-gray-700 font-bold  text-lg'>Name</h2>
                    <input
                        ref={NameRef}
                ...
                <button
                    className=' 
                    flex justify-center items-center gap-3 bg-slate-700 p-3 w-full mt-8 rounded text-white font-bold border-slate-400 transition-all border-b-4  hover:bg-white hover:text-slate-500 text-xl' type={'submit'} >Sign in <ClipLoader color='inherit' loading={isLoading} size={20} /> </button>
                <div className='text-md text-gray-700 flex  w-full justify-start gap-1'>
                    <p >already have an log in here</p>
                    <Link to='/LogIn'>
